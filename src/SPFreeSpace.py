...
import visual.graph as graph
from banded import banded

print("Would you like to display real time probability of finding the particle in region I & II?(yes/no)") 
Ans=raw_input()
...
def v(phi,b1,b2,bv1,bv2):
    ...
    v[(N/2)-1,0]=b1*phi[(N/2)-1,0]+bv2*phi[(N/2),0]+b2*phi[(N/2)-2,0]
    v[N/2,0]=bv1*phi[N/2,0]+bv2*phi[(N/2)+1,0]+b1*phi[(N/2)-1,0]
...
A[0,0:(N/2)-1],A[1,0:(N/2)-1],A[2,0:(N/2)-1]=a2,a1,a2
A[0,N/2],A[1,N/2],A[2,N/2]=a2,av1,av2
A[0,(N/2)+1:],A[1,(N/2)+1:],A[2,(N/2)+1:]=av2,av1,av2    
...
while True:
    ...
    if Ans=='yes':
          ...
          f3.plot(pos=(t,R),color=color.red)
          f4.plot(pos=(t,T),color=color.blue)
