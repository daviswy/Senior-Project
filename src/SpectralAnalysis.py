...
#Calculating the frequency step for the x-axis of the fourier transform.
df=1./a/N
'''
...
#funciotn definition for the fourier transform of data
def dft(y):
    N=len(y)
...
if p==0:
    ...
    while True:
        ...
        #phif=np.fft.fft(abs(phi[:,0]))
        t+=h
        if t>(3*8.4e-16):
            FT=dft(phi[:,0])
            for i in range(N//2+1):
                f1.plot(pos=(kx[i],abs(FT[i])**2))
                break
